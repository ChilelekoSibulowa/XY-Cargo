...
const AgentRequestDelivery = () => {
  ...
  useEffect(() => {
    const fetchRows = async () => {
      ...
      const customersRes = await supabase.from("customers").select("id").eq("agent_id", user.id);
      if (customersRes.error) {
        toast.error(customersRes.error.message || "Failed to load agent customers.");
        setAvailableRows([]);
        setActiveRows([]);
        setHistoryRows([]);
        setIsLoading(false);
      ...
      if (queryErrors.length > 0) {
        console.error("[AgentRequestDelivery] Query errors:", queryErrors);
  ...
  const requestDelivery = async (row: RequestRow) => {
    ...
    const requestPayload = buildDeliveryRequestPayload("agent", user.id);
    ...
    if (row.customer_id) {
      ...
      notifyDeliveryRequested(row.customer_id, row.custom_tracking_number, row.id);
    }
    setUpdatingId(null);
