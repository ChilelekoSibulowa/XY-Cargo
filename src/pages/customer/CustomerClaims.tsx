...
const CustomerClaims = () => {
  ...
  const loadData = async () => {
    ...
    const [shipmentsRes, sourcingRes, supplierPaymentsRes, requestsRes] = await Promise.all([
      supabase
        .from("shipments")
        .select("id, code, status, custom_tracking_number, notes, description, total_cost, shipping_cost, payment_status, updated_at")
      ...
      supabase
        ...
        .select("id, product_name, quantity, budget, status, created_at")
        .eq("customer_id", customer.id)
      ...
      supabase
        ...
        .select("id, request_code, supplier_name, company_name, amount, currency, total_payable, status, created_at")
        .eq("customer_id", customer.id)
        .eq("status", "completed")
        .order("created_at", { ascending: false })
        .limit(50),
      supabase
        ...
        .select("id, shipment_id, shipment_code, description, requested_amount, status, finance_response_message, created_at, shipment:shipments(code, status, custom_tracking_number, notes, description)")
        .eq("customer_id", customer.id)
  ...
  const refundItems = useMemo<RefundableItem[]>(() => {
    if (refundCategory === "product_sourcing") {
      return paidSourcingRequests.map((request) => ({
    ...
    if (refundCategory === "supplier_payments") {
      return paidSupplierPayments.map((request) => ({
    ...
    return paidShipments.map((shipment) => ({
  ...
  }, [refundCategory, paidShipments, paidSourcingRequests, paidSupplierPayments]);
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedRefundItem) {
      toast.error(`Select a ${itemSelectLabel.toLowerCase()} first.`);
      return;
    }
    if (!reason.trim()) {
  ...
  return (
    <CustomerProfileGate>
      <div className="space-y-6">
        <PageHeader title="Request Refund"  />
        ...
        <Card className="border-border/70">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedRefundItemId} onValueChange={setSelectedRefundItemId}>
                <SelectTrigger>
                  <SelectValue placeholder={isLoading ? `Loading ${itemSelectLabel.toLowerCase()}...` : `Select ${itemSelectLabel.toLowerCase()}`} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                placeholder="Explain why you are requesting the refund."
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                      {formatAmount(Number(request.requested_amount || 0))}
                    </p>
                  </div>
                </div>
