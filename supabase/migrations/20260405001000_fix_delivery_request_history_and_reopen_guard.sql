  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    IF v_is_customer_owner OR v_is_agent_for_customer THEN
      IF NOT (
        ...
        OR (
          ...
          AND NEW.status = 'requested_pickup'
          AND OLD.delivery_request_requested_at IS NULL
        )
      ) THEN
        RAISE EXCEPTION 'Customers and agents can only request delivery (Ready for Collection -> Requested), submit consolidation (Need Action -> Submitted), or approve shipping (Confirm Shipment -> Outgoing Parcels).';
      END IF;
    ELSIF v_can_warehouse THEN
      IF NOT (
        ...
        OR (OLD.status = 'requested_pickup' AND NEW.status = 'assigned')
...
BEGIN
  ...
  IF v_new_status IS DISTINCT FROM v_old_status THEN
    IF v_is_customer_owner OR v_is_agent_for_customer THEN
      IF NOT (
        (v_old_status = 'confirmed' AND v_new_status = 'outgoing')
        OR (
          v_old_status = 'arrived'
          AND v_new_status = 'submitted'
          AND OLD.delivery_request_requested_at IS NULL
        )
      ) THEN
        RAISE EXCEPTION 'Customers and agents can only request delivery (Ready for Collection -> Requested) or move consolidation from Confirm Shipment to Outgoing Parcels.';
      END IF;
    ELSIF v_can_warehouse THEN
      IF NOT (
        (v_old_status = 'submitted' AND v_new_status = 'confirmed')
        OR (v_old_status = 'submitted' AND v_new_status = 'outgoing')
...
SET delivery_request_requested_at = COALESCE(actual_delivery_date, updated_at, created_at)
WHERE delivery_request_requested_at IS NULL
...
WITH consolidation_request_times AS (
  SELECT
    cs.consolidation_id,
    MIN(s.delivery_request_requested_at) AS requested_at
  FROM public.consolidation_shipments cs
  JOIN public.shipments s
    ON s.id = cs.shipment_id
  WHERE s.delivery_request_requested_at IS NOT NULL
  GROUP BY cs.consolidation_id
)
UPDATE public.consolidations c
SET delivery_request_requested_at = crt.requested_at
FROM consolidation_request_times crt
WHERE c.id = crt.consolidation_id
  AND c.delivery_request_requested_at IS NULL;
