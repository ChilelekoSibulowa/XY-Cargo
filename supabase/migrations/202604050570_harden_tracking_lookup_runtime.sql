...
BEGIN
  ...
  BEGIN
    SELECT jsonb_build_object(
      ...
      'carrier_query', COALESCE(NULLIF(BTRIM(s.custom_tracking_number), ''), NULLIF(BTRIM(s.code), '')),
  ...
  BEGIN
    SELECT jsonb_build_object(
      ...
      'code', c.code,
      'status', public.normalize_consolidation_status_value(c.status),
      'tracking_number', COALESCE(NULLIF(BTRIM(c.tracking_code), ''), c.code),
      'airway_bill_number', NULL,
      'created_at', c.created_at,
      'pickup_date', NULL,
      ...
      'status_message', 'Tracking details loaded.',
      'carrier_query', COALESCE(NULLIF(BTRIM(c.tracking_code), ''), NULLIF(BTRIM(c.code), '')),
