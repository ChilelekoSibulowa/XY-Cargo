-- Make tracking lookup resilient for warehouse-entered tracking numbers.
-- Prefer shipment-level matches, but return grouped shipment data when one
-- tracking number points to multiple parcels outside a consolidation.
...
BEGIN
  ...
  BEGIN
    ...
    WHERE public.tracking_lookup_matches(s.code, v_lookup)
       ...
       OR lower(COALESCE(s.notes, '')) LIKE '%' || lower(v_lookup) || '%';
    ...
    WHERE public.tracking_lookup_matches(c.code, v_lookup)
       ...
       OR lower(COALESCE(c.notes, '')) LIKE '%' || lower(v_lookup) || '%'
       OR EXISTS (
         SELECT 1
