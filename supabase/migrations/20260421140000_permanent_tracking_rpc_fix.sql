-- Permanent tracking RPC fix.
-- Redefine helpers and the main tracking lookup in one migration so warehouse
-- tracking numbers, AWB/BL values, and grouped parcel lookups all resolve.
...
AS $$
  SELECT COALESCE(
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Warehouse\s+Tracking\s+Number\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Warehouse\s+Tracking\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Consolidation\s+Tracking\s+Number\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Consolidation\s+Tracking\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Tracking\s+Number\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Tracking\s+No\.?\s*[:\-=]\s*([^|\r\n;]+)'))[1]), '')
  );
$$;
...
AS $$
  SELECT COALESCE(
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)AWB\s*/\s*BL\s+No\.?\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Airway\s+Bill\s*(?:No\.?)?\s*[:\-=]\s*([^|\r\n;]+)'))[1]), ''),
    NULLIF(BTRIM((regexp_match(COALESCE(_notes, ''), '(?im)Bill\s+of\s+Lading\s*(?:No\.?)?\s*[:\-=]\s*([^|\r\n;]+)'))[1]), '')
  );
$$;
...
AS $$
DECLARE
  v_lookup_raw text := NULLIF(BTRIM(COALESCE(_lookup, '')), '');
  v_cand_raw text := NULLIF(BTRIM(COALESCE(_candidate, '')), '');
  v_lookup_norm text;
  v_cand_norm text;
BEGIN
  IF v_lookup_raw IS NULL THEN
    RETURN false;
  END IF;

  IF lower(COALESCE(v_cand_raw, '')) = lower(v_lookup_raw) THEN
    RETURN true;
  END IF;

  v_lookup_norm := public.normalize_tracking_lookup_text(v_lookup_raw);
  v_cand_norm := public.normalize_tracking_lookup_text(v_cand_raw);

  IF v_lookup_norm IS NOT NULL AND v_cand_norm IS NOT NULL THEN
    IF v_cand_norm = v_lookup_norm THEN
      RETURN true;
    END IF;

    IF char_length(v_lookup_norm) >= 6 AND position(v_lookup_norm IN v_cand_norm) > 0 THEN
      RETURN true;
    END IF;

    IF char_length(v_cand_norm) >= 6 AND position(v_cand_norm IN v_lookup_norm) > 0 THEN
      RETURN true;
    END IF;
  END IF;
  ...
  BEGIN
      ...
      FROM (
        SELECT
          ...
          MIN(COALESCE(
            ...
            NULLIF(BTRIM(s.custom_tracking_number), ''),
            NULLIF(BTRIM(s.code), '')
          ) IS NOT NULL) AS shipment_tracking_reference,
          COUNT(DISTINCT COALESCE(
            public.shipment_warehouse_tracking_from_notes(s.notes),
            NULLIF(BTRIM(s.custom_tracking_number), ''),
            NULLIF(BTRIM(s.code), '')
            ...
            NULLIF(BTRIM(s.custom_tracking_number), ''),
            NULLIF(BTRIM(s.code), '')
          ) IS NOT NULL) AS shipment_tracking_reference_count,
          jsonb_agg(
            jsonb_build_object(
              'id', s.id,
              'code', s.code,
    ...
    CROSS JOIN LATERAL (
      ...
      SELECT
        ...
        (ARRAY_AGG(event_source.message ORDER BY event_source.created_at DESC))[1] AS latest_message
      FROM event_source
    ) events
