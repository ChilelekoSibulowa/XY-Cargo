  ...
  BEGIN
    ...
    IF COALESCE(array_length(v_matching_shipment_ids, 1), 0) > 1 THEN
      ...
      FROM (
        SELECT
          ...
          (ARRAY_AGG(s.status::text ORDER BY COALESCE(s.updated_at, s.created_at) DESC, s.created_at DESC, s.id DESC))[1] AS latest_status,
          ...
          CASE
            WHEN COUNT(DISTINCT public.shipsgo_transport_from_service_type(s.service_type::text))
              FILTER (WHERE public.shipsgo_transport_from_service_type(s.service_type::text) IS NOT NULL) = 1
            THEN MAX(public.shipsgo_transport_from_service_type(s.service_type::text))
            ELSE NULL
          ...
          jsonb_agg(
            jsonb_build_object(
              ...
              'service_type', s.service_type::text,
    ...
    IF v_shipment_id IS NOT NULL THEN
      SELECT jsonb_build_object(
        ...
        'shipsgo_transport', public.shipsgo_transport_from_service_type(s.service_type::text),
        'events', COALESCE(events.event_rows, '[]'::jsonb),
    ...
    SELECT jsonb_build_object(
      ...
      'id', c.id,
      'code', c.code,
      'status', public.normalize_consolidation_status_value(c.status::text),
      'tracking_number', COALESCE(
