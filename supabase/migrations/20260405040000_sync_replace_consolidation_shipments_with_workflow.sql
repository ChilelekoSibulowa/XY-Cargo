...
BEGIN
  ...
  SET
    ...
    notes = CASE
      WHEN v_consolidation_code IS NULL OR btrim(v_consolidation_code) = '' THEN notes
      WHEN notes IS NULL OR btrim(notes) = '' THEN 'Consolidation: ' || v_consolidation_code
      WHEN notes ~* '(^|\\|)\\s*Consolidation\\s*:' THEN regexp_replace(
        notes,
        '(^|\\|)\\s*Consolidation\\s*:\\s*[^|]+',
        E'\\1 Consolidation: ' || v_consolidation_code,
        'i'
      )
      ELSE notes || ' | Consolidation: ' || v_consolidation_code
    END
  WHERE id = ANY(v_clean_shipment_ids)
    AND status = 'received';
